...
export async function getWorldCupMatches() {
  ...
  try {
    const response = await fetch(`${ESPN_SCOREBOARD}?dates=${dateKey()}`, { signal: AbortSignal.timeout(8000) });
    ...
    if (!matches.length) throw new Error('World Cup provider returned no matches for today.');
    const value = {
      data: matches,
